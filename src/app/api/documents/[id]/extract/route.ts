...
export async function GET(request: Request, context: Context) {
  try {
    ...
    const status=await extractionJobs.status(id, undefined, params.has("status"));
...
export async function POST(request: Request, context: Context) {
  try {
    ...
    const status=await extractionJobs.status(id, undefined, refresh);
    if("result" in status && !refresh) return Response.json({result:status.result,cached:true});
    if(status.status!=="running" && !process.env.OPENAI_API_KEY?.trim()) throw new DocumentError("На сервере не задан OPENAI_API_KEY.",503);
    const started=await extractionJobs.start(id, undefined, refresh);
    ...
    const cancel=()=>started.job.controller.abort();
