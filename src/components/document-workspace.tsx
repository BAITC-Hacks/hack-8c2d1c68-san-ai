  ...
  return <div className={styles.page}>
    ...
    <main className={styles.main}>
      ...
      <div className={styles.columns}>{(["before", "after"] as const).map((side, index) => {
        ...
        return <section key={side} className={styles.card} aria-label={side === "before" ? "Документы ДО" : "Документы ПОСЛЕ"}>
          ...
          {loading ? <p className={styles.empty}>Загружаем список…</p> : !items.length ? <p className={styles.empty}>Здесь пока нет документов.<br/>Прикрепите первый файл этого комплекта.</p> : <ul className={styles.list}>{items.map((doc) => <li key={doc.id}>
            <div className={styles.fileTop}><FileText size={22}/><div><strong>{doc.name}</strong><small>{(doc.size / 1024).toFixed(1)} КБ · {new Date(doc.created_at).toLocaleString("ru-RU")}</small></div></div>
            <p className={doc.status === "parse_error" ? styles.parseError : styles.state}>{doc.status === "parsed" ? `Текст извлечён · ${doc.fragment_count} фрагментов` : doc.status === "parse_error" ? "Файл сохранён · текст не извлечён" : "Файл сохранён · текст ещё не обработан"}</p>
            {doc.warnings.length > 0 && <details><summary>Ограничения обработки</summary>{doc.warnings.map((warning) => <p key={warning}>{warning}</p>)}</details>}
            <div className={styles.links}><a href={`/api/documents/${doc.id}`}><Download size={14}/>Скачать оригинал</a>{doc.status === "parsed" && <a href={`/api/documents/${doc.id}?view=parsed`}>Текст и источники (JSON)</a>}</div>
          </li>)}</ul>}
        </section>;
      })}</div>
