...
export async function extractionRequest(id: string, method: "GET" | "POST" | "DELETE", signal: AbortSignal, fetcher: typeof fetch = fetch, refresh = false): Promise<ExtractionStatus> {
  const suffix=method==="POST" ? `?background=1${refresh ? "&refresh=1" : ""}` : method==="GET" ? "?status=1" : "";
