...
export class ExtractionJobs {
  ...
  async status(id: string, root = storageRoot()) {
    ...
    return this.jobs.get(`${root}:${id}`)?.state ?? {status:"idle" as const,completed_chunks:0,total_chunks:0};
  }
  async start(id: string, root = storageRoot()) {
    const document = await readDocument(id,true,root);
    const cached = await this.cached(root,id);
    if (cached) return {cached:true as const,result:cached};
    const key=`${root}:${id}`;
